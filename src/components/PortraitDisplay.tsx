...
export default function PortraitDisplay({
  ...
  return (
    <div className="relative w-full max-w-[340px] sm:max-w-[420px] md:max-w-[480px] lg:max-w-[540px] mx-auto aspect-[3/4] flex items-end justify-center">
      {/* Background Soft Glow Aura directly centered behind head/torso */}
      <div 
        className="absolute bottom-10 inset-x-0 mx-auto w-3/4 h-3/4 rounded-full bg-gradient-to-t from-lime-300/40 via-lime-200/30 to-transparent blur-2xl pointer-events-none -z-10" 
        aria-hidden="true" 
      />
      ...
      {!imageError ? (
        <div className="relative w-full h-full portrait-fade-mask flex items-end justify-center">
          <Image
            ...
            className="w-full h-auto max-h-full object-contain object-bottom select-none filter contrast-[1.02] drop-shadow-[0_15px_30px_rgba(0,0,0,0.06)]"
          />
        </div>
      ) : (
